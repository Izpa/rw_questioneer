...
The `home-routes` handler in the `rw-questioneer.routes.home` namespace
...
The `home-page` function will in turn call the `rw-questioneer.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `rw-questioneer.handler` namespace:
...
Request middleware functions are located under the `rw_questioneer.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `rw-questioneer.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `rw-questioneer.db.core` in some other namespace.
